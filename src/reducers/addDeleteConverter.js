import {ADD_CONVERTER,
    ...
    EDIT_CURRENCY_FROM,
    EDIT_CURRENCY_TO,
    EDIT_CONVERT_FROM, RUB} from '../constants/index';
import { converter } from '../utils/converter';
    ...
    const initialState = [{
      ...
      export function listConverters(state = initialState, action) {
        switch (action.type) {
          case ADD_CONVERTER:{
            return [
              ...state, {
                ...
                convertFrom: 1,
                convertTo: converter(action.payload.currencyFrom, action.payload.currencyTo, state.convertFrom),
          ...
          case EDIT_CURRENCY_FROM:{
            const newState = state.slice();
            let convert = newState.filter(el => el.id === action.payload.id)[0];
